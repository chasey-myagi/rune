...
def step_a_handler(input_bytes: bytes) -> bytes:
    """step_a: 给 JSON 加 step_a 字段"""
    data = json.loads(input_bytes)
    data["step_a"] = True
    return json.dumps(data).encode()


def step_c_handler(input_bytes: bytes) -> bytes:
    """step_c: 给 JSON 加 step_c 字段"""
    data = json.loads(input_bytes)
    data["step_c"] = True
    return json.dumps(data).encode()
...
RUNE_HANDLERS = {
    "echo": echo_handler,
    "slow": slow_handler,
    "step_a": step_a_handler,
    "step_c": step_c_handler,
}
    ...
    attach_msg = rune_pb2.SessionMessage(
        attach=rune_pb2.CasterAttach(
            ...
            runes=[
                ...
                rune_pb2.RuneDeclaration(
                    name="step_a", version="1.0.0", description="Flow step A",
                ),
                rune_pb2.RuneDeclaration(
                    name="step_c", version="1.0.0", description="Flow step C",
                ),
            ],
            max_concurrent=10,
        )
