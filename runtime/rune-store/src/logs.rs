...
impl RuneStore {
    pub async fn insert_log(&self, log: &CallLog) -> StoreResult<i64> {
        let conn = self.conn.clone(); let log = log.clone();
    ...
    pub async fn query_logs(&self, rune_name: Option<&str>, limit: i64) -> StoreResult<Vec<CallLog>> {
    ...
    pub async fn cleanup_logs_before(&self, before: &str) -> StoreResult<u64> {
        let conn = self.conn.clone(); let before = before.to_string();
        tokio::task::spawn_blocking(move || { let conn = conn.lock().unwrap(); let deleted = conn.execute("DELETE FROM call_logs WHERE timestamp < ?1", rusqlite::params![before])?; Ok(deleted as u64) }).await?
    }
    ...
    pub async fn call_stats_enhanced(&self) -> StoreResult<(i64, Vec<(String, i64, i64, f64, f64)>)> {
        let conn = self.conn.clone();
