...
impl RuneStore {
    ...
    pub fn insert_task(
        ...
        conn.execute(
            "INSERT INTO tasks (task_id, rune_name, status, input, created_at) VALUES (?1, ?2, ?3, ?4, ?5)",
    ...
    pub fn get_task(&self, task_id: &str) -> StoreResult<Option<TaskRecord>> {
        ...
        let result = stmt.query_row(rusqlite::params![task_id], |row| {
            Ok(TaskRecord {
                task_id: row.get(0)?,
                rune_name: row.get(1)?,
                status: TaskStatus::from_str(&row.get::<_, String>(2)?).unwrap_or(TaskStatus::Pending),
                input: row.get(3)?,
                output: row.get(4)?,
                error: row.get(5)?,
    ...
    /// Update a task's status. Sets started_at when transitioning to running,
    /// and completed_at when transitioning to completed/failed/cancelled.
    /// No-op if the task doesn't exist.
    pub fn update_task_status(
        ...
        match status {
            ...
            TaskStatus::Completed | TaskStatus::Failed | TaskStatus::Cancelled => {
                conn.execute(
                    "UPDATE tasks SET status = ?1, output = ?2, error = ?3, completed_at = ?4 WHERE task_id = ?5",
    ...
    pub fn list_tasks(
        ...
        let mut sql = String::from(
            "SELECT task_id, rune_name, status, input, output, error, created_at, started_at, completed_at FROM tasks WHERE 1=1",
        );
        ...
        if let Some(rn) = rune_name {
            ...
            params.push(Box::new(rn.to_string()));
        }
