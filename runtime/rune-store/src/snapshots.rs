...
impl RuneStore {
    ...
    pub async fn list_snapshots(&self) -> StoreResult<Vec<RuneSnapshot>> {
        let conn = self.conn.clone();
        tokio::task::spawn_blocking(move || { let conn = conn.lock().unwrap(); let mut stmt = conn.prepare("SELECT rune_name, version, description, supports_stream, gate_path, gate_method, last_seen FROM rune_snapshots ORDER BY rune_name")?; let snapshots = stmt.query_map([], |row| Ok(RuneSnapshot { rune_name: row.get(0)?, version: row.get(1)?, description: row.get(2)?, supports_stream: row.get::<_, i32>(3)? != 0, gate_path: row.get(4)?, gate_method: row.get(5)?, last_seen: row.get(6)? }))?.collect::<Result<Vec<_>, _>>()?; Ok(snapshots) }).await?
    }
}
